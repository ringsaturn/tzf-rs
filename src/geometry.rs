// FIXME(ringsaturn): xx never used warning
#![allow(dead_code)]

// Copy from https://rosettacode.org/wiki/Ray-casting_algorithm#Rust
...
impl Edge {
    pub fn new(pt1: (f64, f64), pt2: (f64, f64)) -> Edge {
        Edge {
...
#[derive(Debug)]
pub struct Polygon {
    pub edges: Vec<Edge>, // Polygon has to be created with counter-clockwise coordinates
}

pub fn pt_in_polygon(pt: &Point, poly: &Polygon) -> bool {
    let count = poly
        .edges
        .iter()
...
pub fn ray_intersect_seg(p: &Point, edge: &Edge) -> bool {
    let mut pt = p.clone();
