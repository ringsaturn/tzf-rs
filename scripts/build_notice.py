...
def normalize_product_source(cargo_toml: Path) -> str:
    if tomllib is not None:
        ...
        source = package.get("repository") or package.get("homepage") or package.get("name")
        if source:
...
def is_copyright_template(value: str) -> bool:
    return any(token in value for token in ("{yyyy}", "[yyyy]", "name of copyright owner"))
...
LICENSE_OVERRIDES: dict[str, str] = {
    # tzf-dist ships Rust code under MIT; the bundled geodata is ODbL-1.0.
    # cargo-bundle-licenses picks up the ODbL from the data files, so we
    # correct the code license here and append a separate data entry below.
    "tzf-dist": "MIT",
}
